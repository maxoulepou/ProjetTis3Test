...
public class LectureXML {
  ...
  private static final String repBase = "src/donnees/";
                ...
                switch (event) {
                    ...
                    case XMLStreamConstants.END_ELEMENT:
                        ...
                        if (parser.getLocalName().equals("date")) {
                            ...
                            int heure = Integer.parseInt(donneesCourantes.substring(donneesCourantes.lastIndexOf('-')+1, donneesCourantes.length()));
                            int minute = Integer.parseInt(donneesCourantes.substring(donneesCourantes.lastIndexOf('-')+1, donneesCourantes.length()));
                            
                            date = new Date(minute, heure, jour, mois, annee);
                        }
                        ...
                        if (parser.getLocalName().equals("medecin")) {
                            medecinCourant = new Medecin(nomCourant, prenomCourant, specialiteCourante, numeroTel);
                        }
                        if (parser.getLocalName().equals("nom")) {
                            nomCourant = donneesCourantes;
                        }
                        if (parser.getLocalName().equals("numero")) {
                            numeroTel = donneesCourantes; 
                        }
                        if (parser.getLocalName().equals("patient")) {
                            patientCourant = new Patient(nomCourant, prenomCourant, numeroSecu, adresse);
                        }
  ...
  private void ajouterFiche(Date d, Medecin m, Patient p){
